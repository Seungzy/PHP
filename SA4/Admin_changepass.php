...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    verify_csrf();
    try { handle_password_change($user); set_flash('success', 'Password changed successfully.'); redirect('Admin_home.php'); }
    catch (RuntimeException $exception) { $error = $exception->getMessage(); }
}
render_header('Change password'); render_profile($user, 'Admin_home.php');
?>
